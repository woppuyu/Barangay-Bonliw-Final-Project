...
function renderWeeklyCalendar(){ if(!weeklyCalendarEl) return; const weekStartISO = currentWeekStart.toISOString().split('T')[0]; const weekEndISO = addDays(currentWeekStart,4).toISOString().split('T')[0]; const appointments = (window.myAppointmentsList||[]).map(a=>{ const normDate = (typeof a.appointment_date==='string')? a.appointment_date.substring(0,10): new Date(a.appointment_date).toISOString().split('T')[0]; return {...a, _date:normDate}; }).filter(a=> a._date>=weekStartISO && a._date<=weekEndISO); const dayMap={}; for(let i=0;i<5;i++){ const d=addDays(currentWeekStart,i); const iso=d.toISOString().split('T')[0]; dayMap[iso]=[]; } appointments.forEach(a=>{ if(dayMap[a._date]) dayMap[a._date].push(a); }); const monthYear = currentWeekStart.toLocaleDateString(undefined,{month:'long',year:'numeric'}); calendarMonthLabel.textContent=monthYear; weeklyCalendarEl.innerHTML=''; const SLOT_HEIGHT=40; const HEADER_HEIGHT=50; const timeCol=document.createElement('div'); timeCol.className='time-column'; const timeHeader=document.createElement('div'); timeHeader.className='day-header'; timeHeader.style.height=HEADER_HEIGHT+'px'; timeHeader.textContent='Time'; timeCol.appendChild(timeHeader); USER_HALF_HOUR_SLOTS.forEach(t=>{ const label=document.createElement('div'); label.className='time-slot-label'; label.style.height=SLOT_HEIGHT+'px'; label.textContent=formatTime(t.substring(0,5)); timeCol.appendChild(label); }); weeklyCalendarEl.appendChild(timeCol); for(let i=0;i<5;i++){ const d=addDays(currentWeekStart,i); const iso=d.toISOString().split('T')[0]; const dayCol=document.createElement('div'); dayCol.className='day-column'; const header=document.createElement('div'); header.className='day-header'; header.style.height=HEADER_HEIGHT+'px'; const dayStr=d.toLocaleDateString(undefined,{weekday:'short'}); header.innerHTML=`<span>${dayStr}</span> <strong>${d.getDate()}</strong>`; dayCol.appendChild(header); USER_HALF_HOUR_SLOTS.forEach(t=>{ const cell=document.createElement('div'); cell.className='slot-cell'; cell.style.height=SLOT_HEIGHT+'px'; cell.dataset.time=t; dayCol.appendChild(cell); }); (dayMap[iso]||[]).forEach(a=>{ const baseTime=a.appointment_time.substring(0,5); const slotIndex=USER_HALF_HOUR_SLOTS.findIndex(s=>s.startsWith(baseTime)); if(slotIndex===-1) return; const durationSlots=1; const block=document.createElement('div'); block.className=`appointment-block ${a.status}`; const displayTime=formatTime(a.appointment_time.substring(0,5)); const serviceType = a.service_category || 'Document Request'; block.innerHTML=`<div style='font-weight:600;'>${serviceType}</div><div>${displayTime}</div><div style='font-size:10px;'>${a.status.toUpperCase()}</div>`; block.style.top=`${HEADER_HEIGHT+slotIndex*SLOT_HEIGHT+1}px`; block.style.height=`${durationSlots*SLOT_HEIGHT-4}px`; dayCol.appendChild(block); }); weeklyCalendarEl.appendChild(dayCol); } renderLegend(); }
